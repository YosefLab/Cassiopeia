...
class TestCassiopeiaTree(unittest.TestCase):
    ...
    def test_parsimony_bad_cases(self):
        ...
        with self.assertRaises(CassiopeiaTreeError):
            small_tree.calculate_parsimony(infer_ancestral_characters=False)
    ...
    def test_parsimony_reconstruct_internal_states(self):
        ...
        p = small_tree.calculate_parsimony(infer_ancestral_characters=True)
        self.assertEqual(p, 8)
        p = small_tree.calculate_parsimony(
    ...
    def test_parsimony_specify_internal_states(self):
        ...
        p = small_tree.calculate_parsimony(infer_ancestral_characters=False)
    ...
    def test_likelihood_bad_cases(self):
        small_tree = cas.data.CassiopeiaTree(tree=self.small_net)
        with self.assertRaises(CassiopeiaTreeError):
            small_tree.calculate_irreversible_likelihood(0.5, 0.25)
        ...
        with self.assertRaises(CassiopeiaTreeError):
            small_tree.calculate_irreversible_likelihood(
                0.5,
                0.25,
        ...
        L = small_tree.calculate_irreversible_likelihood(
            0.5,
            0.25,
            use_branch_lengths=False,
            use_internal_character_states=True,
    ...
    def test_likelihood_simple_mostly_missing(self):
        ...
        L = small_tree.calculate_irreversible_likelihood(
            0.5, 0.25, use_branch_lengths=False
        )
    ...
    def test_likelihood_time(self):
        ...
        L = small_tree.calculate_irreversible_likelihood(0.5, 0.25)
        self.assertTrue(np.isclose(L, -21.622010953883887))
    ...
    def test_likelihood_sum_to_one(self):
        ...
        for (
            ...
            for a_, b_ in itertools.product([0, 1, -1, 2], repeat=2):
                ...
                ls.append(np.exp(L))
        self.assertTrue(np.isclose(sum(ls), 1.0))
