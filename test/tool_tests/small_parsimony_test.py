...
class TestSmallParsimony(unittest.TestCase):
    def setUp(self):
        ...
        self.binary_tree = cas.data.CassiopeiaTree(
            tree=binary_tree, cell_meta=basic_meta
        )
        ...
        self.general_tree = cas.data.CassiopeiaTree(
            tree=general_tree, cell_meta=general_meta
        )
    ...
    def test_fitch_hartigan_bottom_up(self):
        ...
        fitch_tree = fitch_hartigan_bottom_up(
            self.binary_tree, "nucleotide", copy=True
        )
        ...
        fitch_hartigan_bottom_up(
            self.binary_tree, "nucleotide", add_key="possible_states"
        )
        for n in self.binary_tree.depth_first_traverse_nodes():
            self.assertRaises(
                CassiopeiaTreeError, self.binary_tree.get_attribute, n, "S1"
            )
    ...
    def test_fitch_hartigan_top_down(self):
        ...
        fitch_hartigan_top_down(
            self.binary_tree, label_key="nucleotide_assignment"
        )

        for n in self.binary_tree.depth_first_traverse_nodes():
            self.assertRaises(
                CassiopeiaTreeError, self.binary_tree.get_attribute, n, "label"
            )
            node_states = self.binary_tree.get_attribute(
                n, "nucleotide_assignment"
            )
            self.assertCountEqual(node_states, expected_labels[n])
