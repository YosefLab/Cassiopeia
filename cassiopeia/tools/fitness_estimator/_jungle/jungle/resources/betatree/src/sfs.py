...
class SFS(betatree):
    ...
    def glob_trees(self, ntrees=10):
        ...
        for ti in range(ntrees):
            ...
            self.alleles.append(
                [
                    (clade.weight, clade.branch_length)
    ...
    def getSFS(self, ntrees=10):
        ...
        for ti in range(ntrees):
            self.coalesce()
            for clade in (
                self.BioTree.get_terminals() + self.BioTree.get_nonterminals()
            ):
    ...
    def binSFS(self, mode="logit", bins=10):
        ...
        if np.iterable(bins):
            ...
            if mode == "logit":
                nbins = np.exp(
                    0.9 * np.log(self.n) * np.linspace(-1, 1, bins + 1)
                )
            ...
            if mode == "log":
                nbins = np.exp(
                    0.9 * np.log(self.n) * np.linspace(-1, 0, bins + 1)
                )
                self.bin_center = np.sqrt(nbins[1:] * nbins[:-1])

        self.bin_width = nbins[1:] - nbins[:-1]

        self.binned_sfs, tmp = np.histogram(
            np.linspace(0, 1, self.n + 1), weights=self.sfs, bins=nbins
        )
    ...
    def loadSFS(self, fname, alpha=None):
        ...
        if len(tmp.shape) == 1:
            ...
            print(
                (
                    "expect a one dimensional vector, got object with shape",
                    tmp.shape,
                )
            )
...
if __name__ == "__main__":
    ...
    for alpha in [2, 1.5, 1]:
        ...
        if calc:
            ...
            mySFS.saveSFS(
                "example_SFS/sfs_"
                + "_".join(map(str, ["alpha", alpha, "n", n]))
                + file_ending
            )
        else:
    ...
    plt.xticks(logit(tick_locs), list(map(str, tick_locs)))
    plt.legend(loc=9)
    plt.xlabel("derived allele frequency")
