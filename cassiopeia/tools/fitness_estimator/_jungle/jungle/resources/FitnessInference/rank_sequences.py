...
parser.add_argument(
    ...
    default=0.0625,
    type=float,
    help="time scale for local tree length estimation (relative to average pairwise distance)",
)
parser.add_argument(
    "--collapse",
    const=True,
...
print(
    (
        "Using time scale tau =",
        params.tau,
        "in units of the average pairwise distance between sequences",
    )
)
...
if params.plot:
    tree_utils.plot_prediction_tree(
        prediction, method="polarizer", internal=True
    )
