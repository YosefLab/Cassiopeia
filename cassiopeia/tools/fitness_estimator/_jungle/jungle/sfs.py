...
class SFS:
    ...
    def bin(self, bins, n_bins=None, min_edge=None, max_edge=None):
        ...
        # Perform binning
        freqs = np.arange(1, len(self.counts) + 1) / float(
            len(self.counts)
        )  # frequency of each count index (i/n_leaves)
        freq_bins = np.digitize(
            freqs, self.bin_edges, right=True
        )  # bin that each frequency belongs to
        self.binned = np.bincount(
            freq_bins, weights=self.counts, minlength=len(self.bin_centers)
        )  # sum mutations in each bin
        ...
        self.binned_normalized = (
            self.binned / self.bin_sizes
        )  # normalize by size of bin
        ...
        if self.T is not None:
            ...
            self.binned_normalized_cut[
                self.bin_edges[1:] < 1 / float(len(self.T))
            ] = np.nan
            self.binned_normalized_cut[
                self.bin_edges[:-1] > 1 - (1 / float(len(self.T)))
            ] = np.nan
    ...
    def _calculate_fay_and_wus_H(self):
        ...
        idx = np.array(
            list(range(n))
        )  # number of individuals sharing a mutation
