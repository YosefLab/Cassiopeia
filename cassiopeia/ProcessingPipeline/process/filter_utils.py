...
def record_stats(moleculetable: pd.DataFrame) -> (np.array, np.array, np.array):
    """
...
def filter_cell_bcs(
    ...
    for n, group in tqdm(
        moleculetable.groupby(["cellBC"]), desc="Filter cells"
    ):
...
def error_correct_intbc(
    ...
    Returns:
        moleculetable: filtered allele table with bad UMIs thrown out/error corrected
    """
