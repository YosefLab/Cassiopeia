...
from cassiopeia.ProcessingPipeline.process import call_lineage_utils as cl_utils
...
def filter_alignments(
    ...
    filtered_df = filter_utils.filter_cell_bcs(
...
def call_lineage_groups(
    ...
    at = cl_utils.annotate_lineage_groups(input_df, kinship_scores, master_intBCs)
    ...
    filtered_lgs = cl_utils.filter_intBCs_final_lineages(at, min_intbc_thresh=min_intbc_thresh)

    at = cl_utils.filtered_lineage_group_to_allelle_table(filtered_lgs)
    ...
    logging.info("Filtering out low UMI cell barcodes...")
    at = filter_utils.filter_cell_bcs(
        at,
        output_directory,
