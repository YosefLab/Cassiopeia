...
class MaxCutSolver(GreedySolver.GreedySolver):
    ...
    def perform_split(self, samples: List[int] = None) -> Tuple[List[int], List[int]]:
        """
        """
        G = graph_utils.construct_connectivity_graph(self.prune_cm, samples, self.missing_char, w = self.weights)
        ...
        improved_S = graph_utils.max_cut_improve_cut(G, return_set)

        rest = set(samples) - improved_S

        return list(improved_S), list(rest)

    def evaluate_cut(self, S, G, B=None, show_total=False):
        cut_score = 0
        total_good = 0
        total_bad = 0
        for e in G.edges():
            ...
            if graph_utils.check_if_cut(u,v,S):
