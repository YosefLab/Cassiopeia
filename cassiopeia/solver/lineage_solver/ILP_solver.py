...
try:
    ...
    warnings.warn(
        "No module called gurobipy found. ILP solver will exit automatically if used."
    )
...
def solve_steiner_instance(
    ...
    subgraphs = retreive_and_print_subgraph(
        model, graph, edge_variables, detailed_output
    )
    ...
    if detailed_output:
        print(
            "Steiner tree solving took %s days, %s hours, %s minutes, %s seconds"
...
def generate_mSteiner_model(graph, source, destinations):
    ...
    for u, v in graph.edges():
        edge_variables_binary[u, v] = model.addVar(
            vtype=GRB.BINARY, name="edge_binary_%s_%s" % (u, v)
        )
...
def retreive_and_print_subgraph(model, graph, edge_variables, detailed_output):
    ...
    for i in range(0, model.SolCount):
        ...
        if (
            True or model.status == GRB.status.OPTIMAL
        ):  # AW: I removed the restriction that requires only an optimal model
            ...
            if detailed_output:
                print(
                    "Solved Steiner Tree instance. Optimal Solution costs "
                    + str(model.PoolObjVal)
                )
...
def execution_time(start_time, end_time):
    ...
    return (
        execution_delta.days,
        execution_delta.seconds // 3600,
        (execution_delta.seconds // 60) % 60,
        execution_delta.seconds % 60,
    )
