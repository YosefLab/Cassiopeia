...
def construct_connectivity_graph(cm, missing_char, samples = None, w = None):
    ...
    for i in samples:
        G.add_node(i)
    F = utils.compute_mutation_frequencies(cm, samples)
    for i, j in itertools.combinations(samples, 2):
        ...
        for l in range(cm.shape[1]):
            ...
            if (x != missing_char and y!= missing_char) and (x != '0' or y != '0'):
                if w is not None:
                    if x==y:
                        score -= 3 * w[l][x] * (len(samples) - F[l][x] - F[l][missing_char])
                    elif x == '0' or y == '0':
...
def spectral_improve_cut(S, G, display=False):
    ...
    num =  sum([G[e[0]][e[1]]['weight'] for e in G.edges() if check_if_cut(e[0], e[1], new_S)])
    ...
    for u in G.nodes():
        ...
        if d == 0:
            return [u]
        c = sum([G[u][v]['weight'] for v in G.neighbors(u) if check_if_cut(u,v,new_S)])
        delta_n[u] = d-2*c
        if u in new_S:
            delta_d[u] = -d
        else:
...
def similarity(u, v, cm, missing_char, w = None):
    ...
    k = cm.shape[0]
    if w is None:
        return sum([1 for i in range(k) if cm.iloc[u, i] == cm.iloc[v, i] and (cm.iloc[u, i] != '0' and cm.iloc[u, i] != missing_char)])
    else:
        return sum([w[i][cm.iloc[u, i]] for i in range(k) if if cm.iloc[u, i] == cm.iloc[v, i] and (cm.iloc[u, i] != '0' and cm.iloc[u, i] != missing_char)])
