...
class Cassiopeia_Tree:
    ...
    def sample_alternative_solutions(self, maximum_alt_solutions=100):
        ...
        target_charstrings = [
            (n.char_string, n.pid) for n in self.network if n.is_target
        ]

        num_solutions = np.prod(
            [
                len(self.alternative_solutions[r])
        ...
        while num_considered_solutions < min(num_solutions, maximum_alt_solutions):
            ...
            if tuple(current_sol) not in sol_identifiers:
                ...
                # assign targets
                seen_charstrings = [
                    n.char_string for n in self.base_network if n.is_target
                ]
                for n in new_network:

                    if (
                        (n.char_string, n.pid) in target_charstrings
                        and n.char_string not in seen_charstrings
                    ):
                        n.is_target = True
