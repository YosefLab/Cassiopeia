...
def get_net_div(time, n, epsilon):
    ret = []
    test = robjects.r('''
    require(geiger, lib.loc = '~/richardz/anaconda2/envs/python3_6/lib/R/library')
    epsilon = ''' + str(epsilon) + '''
...
def count_edge_mutations(network):
    num_muts = []
    root = [n for n in network if network.in_degree(n) == 0][0]
    for i in network.successors(root):
        count_edge_mutations_helper(network, i, num_muts, root.char_vec, 1)
    return num_muts

def count_edge_mutations_helper(network, node, muts_dist, parent_vec, depth):
    mut_count = 0
    poss_count = 0
    for i in range(len(node.char_vec)):
        if node.char_vec[i] != '*' and node.char_vec[i] != '-':
            if parent_vec[i] == '0':
                poss_count += 1
            if node.char_vec[i] != parent_vec[i]:
                mut_count += 1
    
    muts_dist.append((mut_count, poss_count, depth))
