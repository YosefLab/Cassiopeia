...
def find_split(nodes, priors=None, considered=set(), fuzzy=False, probabilistic=False, minimum_allele_rep = 1.0):
	...
	for node in nodes:
		...
		for i in range(0, len(node_list)):
			...
			if (str(i), char) not in considered:
				...
				if char != "0" and char != "-":
					if priors:
						character_mutation_mapping[(str(i), char)] -= np.log(priors[int(i)][char])
					else:
	...
	if probabilistic:
		...
		entries, vals = list(character_mutation_mapping.keys()), list(character_mutation_mapping.values())
		...
		for i, j in character_mutation_mapping:
			...
			if (max_cost < (character_mutation_mapping[(i, j)] + epsilon) and missing_value_prop[str(i)] < minimum_allele_rep):
...
def classify_missing_value(node, left_split, right_split, knn_neighbors, knn_distances, theta = 0.1, kernel=True, mode = 'knn',
							 lookahead_depth = 3, left_states = [], right_states = []):
	"""
	...
	if mode == 'knn':
		...
		raise Exception("Classification method not recognized. Please choose from: lookahead, knn, avg, modified_avg")
...
def perform_split(nodes, character, state, knn_neighbors, knn_distances, considered, missing_data_mode = 'lookahead', lookahead_depth = 3):
	"""
	...
	if missing_data_mode == 'lookahead':
		...
		left_states = look_ahead_helper(left_split, lookahead_depth, dict(), considered.copy())
		right_states = look_ahead_helper(right_split, lookahead_depth, dict(), considered.copy())
	...
	for node, score in sorted(NA_dict.items(), key = lambda kv: kv[1]):
		...
		if classify_missing_value(node, left_split, right_split, knn_neighbors, knn_distances, theta=0.1, kernel=True, 
									mode = missing_data_mode, lookahead_depth = lookahead_depth, left_states = left_states, right_states = right_states):
			right_split.append(node)
		else:
			left_split.append(node)
	...
	if depth == 0 or len(targets) == 1 or len(targets) == 0:
		...
		for node in targets:
			...
			if node_list[character] == state:
				...
				left_split.append(node)
		left_states = look_ahead_helper(left_split, depth - 1, splits.copy(), considered.copy())
		right_states = look_ahead_helper(right_split, depth - 1, splits.copy(), considered.copy())
