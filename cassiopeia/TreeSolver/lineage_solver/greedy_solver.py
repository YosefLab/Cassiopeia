...
def classify_missing_value(node, left_split, right_split, knn_neighbors, knn_distances, theta = 0.1, kernel=True, mode = 'knn'):
	...
	if mode == 'knn':
		...
		raise Exception("Classification method not recognized. Please choose from: knn, avg, modified_avg")
...
def perform_split(nodes, character, state, knn_neighbors, knn_distances):
	"""
...
def greedy_build(
	...
	left_split, right_split = perform_split(nodes, character, state, knn_neighbors, knn_distances)
