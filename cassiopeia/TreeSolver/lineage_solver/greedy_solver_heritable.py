...
def find_split(
    ...
    for node in nodes:
        ...
        for i in range(0, len(node_list)):
            ...
            if (char == "-" or char == "*") and (curr_depth >= heritable_depth):
                missing_value_prop[str(i)] += 1.0 / len(nodes)

            if (str(i), char) not in considered:
                # you can't split on a 'None' state
                if curr_depth >= heritable_depth:
                    if char != "0":
                        if char == "*":
                            if priors:
                                character_mutation_mapping[(str(i), "-")] -= np.log(
                                    priors[int(i)][char]
                                )
                            else:
                                character_mutation_mapping[(str(i), "-")] += 1
                        else:
                            if priors:
...
def perform_split(
    ...
    if state == "-" or state == "*":
        ...
        # print([x.split("|")[character] for x in left_split])
        # print([x.split("|")[character] for x in right_split])

        if character in h_drops:
            h_drops[character].append((h_num, s_num))
        else:
            h_drops[character] = [(h_num, s_num)]
