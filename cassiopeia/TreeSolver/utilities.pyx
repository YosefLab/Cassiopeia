...
def fill_in_tree(tree, cm = None):
	...
	anc_dct = {}
	for n in tqdm(nx.dfs_postorder_nodes(tree, root), total = len([n for n in tree.nodes])):
...
def convert_network_to_newick_format(graph, use_intermediate_names = False):
	...
	def _to_newick_str(g, node):
		...
		return '%s' % (_name,) + ":1" if is_leaf else (
...
def create_bootstrap_from_alleletable(at, no_context = False, mutation_map = None, B = 100):
	...
	lineage_profile = alleletable_to_lineage_profile(at, no_context = no_context, write=False)
	...
	for b in tqdm(range(B), desc = "drawing bootstrap samples"):
		...
		cm_b, prior_probs, indel_to_charstate = lineage_profile_to_charmat(b_sample, mutation_map = mutation_map)
...
def lineage_profile_to_charmat(lp, mutation_map = None):
	...
	cols_to_unique = dict(zip(lp.columns, [lp[x].factorize()[1].values for x in lp.columns]))
	...
	cm = lp.apply(lambda x: [mut_to_state[x.name][v] if type(v) == str else '-' for v in x.values], axis=0)
