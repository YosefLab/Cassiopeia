...
def calculate_cPHS(
    ...
    .. warning::
        ...
        The cPHS is strongly influenced by the accuracy of the ancestral
        character reconstruction. :func:`cassiopeia.tl.ancestral_characters`
        uses Camin-Sokal parsimony, which is only optimal for a perfect tree
        reconstruction and a character matrix with no observation noise.
        Reconstructing ancestral states with the Sankoff algorithm
        (``pycea.tl.ancestral_states(method="sankoff")``) can improve results.
        However, it is **not recommended** to compare different tree-reconstruction
        algorithms using cPHS, since the statistic conflates errors in the
        topology with errors in the ancestral state reconstruction.
