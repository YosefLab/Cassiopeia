...
def create_output_dir(outputdir = None):
    ...
    with open(outputdir + "/eclog_intbc.txt", "w") as f:
        f.write("CellBC\tIntBC(R)\tIntBC(C)\tN_UMI(R)\tN_UMI(C)\n")
...
def record_stats(moleculetable, outputdir, stage="Init"):
    ...
    for n, g in tqdm(moleculetable.groupby(["cellBC"])):
        x = g.groupby(["intBC"]).agg({"UMI": "count"})["UMI"]
        if x.shape[0] > 0:
            ...
            for r1 in range(x1.shape[0]):
                ...
                for r2 in range(r1 + 1, x1.shape[0]):
                    ...
                    if bclDist <= bcDistThresh and allele1 == allele2:
                        ...
                        #     umiCounts = x1.loc[[r1, r2], "UMI"]
                        #
                        #     # set to bad status if a intBC doesn't pass the allele proportion and umiCountThresh criteria
                        #     if  props[r2] <= propThresh and umiCounts[r2] <= umiCountThresh:
                        #         bad_locs = moleculetable[
                        #             (moleculetable["cellBC"] == name) & (moleculetable["intBC"] == iBC2) &
                        #             (moleculetable["allele"] == allele2)]
                        #         for bl in bad_locs.index.values:
                        #             index_filter[bl] = "bad"
                        #         unrecovered += len(bad_locs.index.values);
...
if __name__ == "__main__":
    ...
    stages = ["Init", "EC_UMI", "Filtered_UMI", "Process_intBC", "MappedIntBC"]

    # Plot Read Per UMI Histogram
    h = plt.figure(figsize=(14, 10))
    for n in stages:
        ax = plt.hist(rc_profile[n], label=n, histtype="step", log=True)
    plt.legend()
    ...
    h = plt.figure(figsize=(14, 10))
    for n in stages:
        ax = plt.hist(upi_profile[n], label=n, histtype="step", log=True)
    plt.legend()
    plt.ylabel("Frequency")
    plt.xlabel("Number of UMIs")
