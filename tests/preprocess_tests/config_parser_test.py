...
class TestCollapseUMIs(unittest.TestCase):
    ...
    def test_read_good_config(self):
        ...
        self.assertFalse(parameters["error_correct_umis"]["allow_allele_conflicts"])
        self.assertEqual(parameters["align"]["gap_open_penalty"], 20)
        self.assertEqual(parameters["call_alleles"]["cutsite_width"], 12)
        self.assertFalse(parameters["filter_molecule_table"]["allow_allele_conflicts"])
        ...
        self.assertEqual(parameters["call_alleles"]["barcode_interval"], (10, 25))
