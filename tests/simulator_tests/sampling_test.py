...
class TestSampleSpatial:
    ...
    def test_spatial_key_param(self):
        """spatial_key parameter selects the correct obsm entry."""
        tdata = _make_tdata(n_leaves=4)
        leaves = list(tdata.obs_names)
...
class TestSampleSupercellular:
    ...
    # --- Pixel mode (spatial_key) ---

    def test_pixel_mode_bad_params_with_ratio(self):
        tdata = _make_spatial_tdata(n_leaves=4)
        with pytest.raises(LeafSubsamplerError):
            sample_supercellular(tdata, spatial_key="spatial", ratio=0.5)

    def test_pixel_mode_bad_params_with_n_merges(self):
        tdata = _make_spatial_tdata(n_leaves=4)
        with pytest.raises(LeafSubsamplerError):
            sample_supercellular(tdata, spatial_key="spatial", number_of_merges=1)

    def test_pixel_mode_missing_spatial_key(self):
        tdata = _make_tdata(n_leaves=4)
        with pytest.raises(LeafSubsamplerError):
            sample_supercellular(tdata, spatial_key="spatial")

    def test_pixel_mode_no_overlap(self):
    ...
    def test_pixel_mode_with_overlap(self):
    ...
    def test_pixel_mode_characters_combined(self):
        tdata = _make_tdata(n_leaves=4, with_chars=False)
        leaves = list(tdata.obs_names)
        tdata.obsm["characters"] = pd.DataFrame({"c1": ["1", "2", "3", "4"]}, index=leaves)
        tdata.obsm["spatial"] = pd.DataFrame(
    ...
    def test_pixel_mode_collapse_duplicates(self):
        tdata = _make_tdata(n_leaves=4, with_chars=False)
        leaves = list(tdata.obs_names)
        tdata.obsm["characters"] = pd.DataFrame({"c1": ["1", "1", "3", "4"]}, index=leaves)
        tdata.obsm["spatial"] = pd.DataFrame(
    ...
    def test_pixel_mode_no_collapse_duplicates(self):
        tdata = _make_tdata(n_leaves=4, with_chars=False)
        leaves = list(tdata.obs_names)
        tdata.obsm["characters"] = pd.DataFrame({"c1": ["1", "1", "3", "4"]}, index=leaves)
        tdata.obsm["spatial"] = pd.DataFrame(
    ...
    def test_pixel_mode_tree_valid(self):
        tdata = _make_tdata(n_leaves=4)
        leaves = list(tdata.obs_names)
        tdata.obsm["spatial"] = pd.DataFrame(
            [[1, 1], [1, 1], [2, 2], [3, 3]], index=leaves, columns=["x", "y"]
    ...
    def test_pixel_mode_composed_after_spatial(self):
        ...
        # Put first 4 leaves at same pixel
        ...
        filtered = sample_spatial(tdata, space=space)
        merged = sample_supercellular(filtered, spatial_key="spatial")
        # 4 at same pixel → 1 merged + 4 others = 5 total
