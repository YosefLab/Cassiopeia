...
class SpatialLeafSubsamplerTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for node in tree.leaves:
            tree.set_attribute(node, "spatial", tree.get_attribute(node, "spatial")[:2])
        self.tree_2d = tree
        # 3D regions
    ...
    def test_bad_init_parameters(self):
        ...
        with self.assertRaises(LeafSubsamplerError):
            SpatialLeafSubsampler(ratio=0.5, number_of_leaves=1, space=self.space_2d)
        # neither space nor bounding_box provided
        with self.assertRaises(LeafSubsamplerError):
            SpatialLeafSubsampler()
        # both space and bounding_box provided
        with self.assertRaises(LeafSubsamplerError):
            SpatialLeafSubsampler(space=self.space_2d, bounding_box=self.bounding_box_2d)
        # negative number of leaves
        with self.assertRaises(LeafSubsamplerError):
            SpatialLeafSubsampler(number_of_leaves=-1, space=self.space_2d)
        # ratio > 1
        with self.assertRaises(LeafSubsamplerError):
            SpatialLeafSubsampler(ratio=2, space=self.space_2d)
        # merge cells without space
        with self.assertRaises(LeafSubsamplerError):
            SpatialLeafSubsampler(bounding_box=self.bounding_box_3d, merge_cells=True)
    ...
    def test_bad_subsample_parameters(self):
        ...
        with self.assertRaises(LeafSubsamplerError):
            sampler = SpatialLeafSubsampler(number_of_leaves=100, space=self.space_3d)
    ...
    def test_bad_scale(self):
        ...
        tree = CassiopeiaTree(tree=balanced_tree)
        tree.set_attribute("node1", "spatial", (0.001, 0))
        tree.set_attribute("node2", "spatial", (0, 0.001))
        sampler = SpatialLeafSubsampler(space=self.space_2d)
    ...
    def test_merge_cells(self):
        ...
        self.assertEqual(res.get_attribute("node3-node4", "character_states"), [(0, 1), (1,)])
        self.assertEqual(res.get_attribute("node5", "character_states"), [(1), (1)])
        # without collapsing duplicates
        res = sampler.subsample_leaves(tree, collapse_duplicates=False)
        self.assertEqual(res.get_attribute("node3-node4", "character_states"), [(0, 1), (1, 1)])
        self.assertEqual(res.get_attribute("node5", "character_states"), [(1), (1)])
