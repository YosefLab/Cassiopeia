...
class TestCas9LineageTracingDataSimulator(unittest.TestCase):
    ...
    def test_setup_errors(self):
        ...
        with self.assertRaises(DataSimulatorError):
            cas.sim.Cas9LineageTracingDataSimulator(number_of_cassettes=0, size_of_cassette=2)

        with self.assertRaises(DataSimulatorError):
            cas.sim.Cas9LineageTracingDataSimulator(number_of_cassettes=0.1, size_of_cassette=2)

        # test size of cassette is not a positive integer
        with self.assertRaises(DataSimulatorError):
            cas.sim.Cas9LineageTracingDataSimulator(number_of_cassettes=2, size_of_cassette=0)

        with self.assertRaises(DataSimulatorError):
            cas.sim.Cas9LineageTracingDataSimulator(number_of_cassettes=2, size_of_cassette=0.1)
    ...
    def test_introduce_states(self):
        ...
        for i in range(len(expected_character_array)):
            self.assertEqual(expected_character_array[i], updated_character_array[i])
    ...
    def test_collapse_sites(self):
        ...
        (
            ...
            remaining_cuts,
        ) = self.basic_lineage_tracing_data_simulator.collapse_sites(character_array, [0, 1])
        ...
        (
            updated_character_array,
            remaining_cuts,
        ) = self.basic_lineage_tracing_data_simulator.collapse_sites(character_array, [0, 5, 9])
        ...
        (
            updated_character_array,
            remaining_cuts,
        ) = self.lineage_tracing_data_simulator_missing_data.collapse_sites(character_array, [0, 1])
        self.assertCountEqual([], remaining_cuts)
    ...
    def test_no_collapse(self):
        self.basic_lineage_tracing_data_simulator_no_collapse.overlay_data(self.basic_tree)
        character_matrix = self.basic_tree.character_matrix
    ...
    def test_simulator_with_per_character_priors(self):
        ...
        sim_from_array_len_3 = cas.sim.Cas9LineageTracingDataSimulator(
            ...
            size_of_cassette=3,
            number_of_states=4,
            state_priors=[dict.fromkeys(range(4), 0.25)] * 3,
        )
        ...
        sim_from_array_len_6 = cas.sim.Cas9LineageTracingDataSimulator(
            ...
            size_of_cassette=3,
            number_of_states=4,
            state_priors=np.array([dict.fromkeys(range(4), 0.25)] * 6),
        )
    ...
    def test_simulator_with_per_character_rates(self):
        sim_from_float = cas.sim.Cas9LineageTracingDataSimulator(
            number_of_cassettes=2, size_of_cassette=3, number_of_states=4, mutation_rate=0.1
        )

        sim_from_array_len_3 = cas.sim.Cas9LineageTracingDataSimulator(
            number_of_cassettes=2, size_of_cassette=3, number_of_states=4, mutation_rate=[0.1] * 3
        )
