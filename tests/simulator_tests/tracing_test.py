...
def test_bad_sequential_params(tdata):
    ...
    with pytest.raises(DataSimulatorError):
        stochastic_tracing(tdata, initiation_rate=0.1, continuation_rate=0.1,
                           state_priors={1: 0.6, 2: 0.6})  # priors don't sum to 1
    with pytest.raises(DataSimulatorError):
        stochastic_tracing(tdata, initiation_rate=0.1, continuation_rate=-0.1,
                           state_priors=SIMPLE_PRIORS)  # bad continuation rate
...
def test_cas9_state_values(tdata):
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
...
def test_cas9_inheritance(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, random_seed=2)
...
def test_cas9_leaf_obsm_matches_node_attrs(tdata):
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, random_seed=3)
    tree = tdata.obst["tree"]
    df = tdata.obsm["characters"]
...
def test_cas9_state_generating_distribution(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=3,
                       state_priors=None, number_of_states=5, random_seed=4)
    df = tdata.obsm["characters"]
...
def test_cas9_per_cassette_priors(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       mutation_rate=100.0, state_priors=priors, random_seed=5)
    df = tdata.obsm["characters"]
...
def test_cas9_per_character_rates(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       mutation_rate=rates, state_priors=SIMPLE_PRIORS, random_seed=10)
    df = tdata.obsm["characters"]
...
def test_cas9_collapse_sites(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=3,
                       mutation_rate=1000.0, state_priors=SIMPLE_PRIORS, random_seed=7)
    missing_data(tdata, heritable_missing_rate=0.0, stochastic_missing_rate=0.0,
                 collapse_sites_on_cassette=True, random_seed=7)
    df = tdata.obsm["characters"]
...
def test_cas9_no_collapse(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       mutation_rate=1000.0, state_priors=SIMPLE_PRIORS, random_seed=8)
    df = tdata.obsm["characters"]
    assert "-" not in set(df.values.flatten())

def test_cas9_custom_characters_key(tdata):
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=2,
                       state_priors={1: 1.0}, characters_key="edits", random_seed=0)
...
def test_uns_storage(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=4,
                       state_priors=SIMPLE_PRIORS, random_seed=0)
...
def test_custom_unmodified_state(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=3,
                       mutation_rate=1000.0, state_priors=SIMPLE_PRIORS,
                       unmodified_state="WT", random_seed=7)
...
def test_collapse_custom_missing_state(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=3,
                       mutation_rate=1000.0, state_priors=SIMPLE_PRIORS, random_seed=7)
    missing_data(tdata, heritable_missing_rate=0.0, stochastic_missing_rate=0.0,
                 collapse_sites_on_cassette=True, missing_state="DEL", random_seed=7)
    df = tdata.obsm["characters"]
    all_vals = set(df.values.flatten())
...
def test_missing_data_default_missing_state(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, mutation_rate=0.0, random_seed=0)
    missing_data(tdata, heritable_missing_rate=0.0, stochastic_missing_rate=1.0,
                 random_seed=0)
    df = tdata.obsm["characters"]
...
def test_sequential_basic_shape_and_columns(tdata):
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       initiation_rate=0.1, continuation_rate=0.1,
                       state_priors=SIMPLE_PRIORS, random_seed=0)
    df = tdata.obsm["characters"]
    assert df.shape == (8, 6)
    assert list(df.columns) == ["0-0", "0-1", "0-2", "1-0", "1-1", "1-2"]
...
def test_sequential_no_state_priors_required(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=3,
                       initiation_rate=0.5, continuation_rate=0.5,
                       number_of_states=5, random_seed=15)
    df = tdata.obsm["characters"]
    assert set(df.values.flatten()).issubset({"*", "1", "2", "3", "4", "5"})
...
def test_sequential_ordering_constraint(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       initiation_rate=0.5, continuation_rate=0.5,
                       state_priors=SIMPLE_PRIORS, random_seed=20)
    tree = tdata.obst["tree"]
    for node in tree.nodes:
...
def test_sequential_inheritance(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=3,
                       initiation_rate=0.5, continuation_rate=0.5,
                       state_priors=SIMPLE_PRIORS, random_seed=21)
    tree = tdata.obst["tree"]
    for node in tree.nodes:
        for parent in tree.predecessors(node):
...
def test_sequential_high_rate_fills_cassette(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       initiation_rate=1000.0, continuation_rate=1000.0,
                       state_priors=SIMPLE_PRIORS, random_seed=22)
    df = tdata.obsm["characters"]
...
def test_heritable_silencing_on_internal_nodes(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, random_seed=30)
    missing_data(tdata, heritable_missing_rate=10.0, stochastic_missing_rate=0.0,
                 random_seed=30)
    tree = tdata.obst["tree"]
    internal_nodes = [n for n in tree.nodes if tree.out_degree(n) > 0
                      and tree.in_degree(n) > 0]
    all_vals = set()
...
def test_heritable_silencing_propagates(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, random_seed=31)
    missing_data(tdata, heritable_missing_rate=10.0, stochastic_missing_rate=0.0,
                 random_seed=31)
    tree = tdata.obst["tree"]
    for node in tree.nodes:
...
def test_stochastic_silencing_leaves_only(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, mutation_rate=0.0, random_seed=40)
    missing_data(tdata, heritable_missing_rate=0.0, stochastic_missing_rate=1.0,
                 random_seed=40)
    tree = tdata.obst["tree"]
...
def test_missing_data_cassette_level(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, random_seed=50)
    missing_data(tdata, heritable_missing_rate=10.0, stochastic_missing_rate=1.0,
                 random_seed=50)
    tree = tdata.obst["tree"]
    for node in tree.nodes:
        chars = tree.nodes[node]["characters"]
...
def test_obsm_rebuilt_after_missing_data(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, random_seed=60)
    missing_data(tdata, heritable_missing_rate=1.0, stochastic_missing_rate=0.5,
                 random_seed=60)
    tree = tdata.obst["tree"]
    df = tdata.obsm["characters"]
    for leaf in df.index:
...
def test_missing_data_custom_missing_state(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=2,
                       state_priors={1: 1.0}, random_seed=70)
    missing_data(tdata, heritable_missing_rate=10.0, stochastic_missing_rate=1.0,
                 missing_state="X", random_seed=70)
    df = tdata.obsm["characters"]
    all_vals = set(df.values.flatten())
...
def test_missing_data_key_added(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=2, size_of_cassette=3,
                       state_priors=SIMPLE_PRIORS, random_seed=90)
    orig_df = tdata.obsm["characters"].copy()
    missing_data(tdata, heritable_missing_rate=10.0, stochastic_missing_rate=1.0,
                 key_added="characters_missing", random_seed=90)
...
def test_missing_data_uns_storage(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=1, size_of_cassette=2,
                       state_priors={1: 1.0}, random_seed=0)
...
def test_composed_pipeline(tdata):
    ...
    stochastic_tracing(tdata, number_of_cassettes=3, size_of_cassette=2,
                       state_priors=SIMPLE_PRIORS, random_seed=80)
    missing_data(tdata, heritable_missing_rate=0.1, stochastic_missing_rate=0.1,
                 random_seed=80)
    df = tdata.obsm["characters"]
    assert df.shape == (8, 6)
