    ...
    for i in range(m - 1):
        for j in range(i + 1, m):
            ...
            if mSize > 0:
                ...
                dm[k] = dmat.iloc[i, j] / float(mSize)

            k += 1
    return dm

def get_lg_group(df, curr_LG):
    ...
    intbc_counter = pd.DataFrame({"iBC": intbcs, "prop": [0]*len(intbcs)})
    for cb in np.unique(lg_group["cellBC"]):
        # get unique intbcs in this cell
        mols = np.unique(lg_group.loc[lg_group["cellBC"] == cb, "intBC"])
        intbc_counter.loc[(intbc_counter["iBC"]).isin(mols), "prop"] += 1

    n_cells = len(np.unique(lg_group["cellBC"]))

    intbc_counter["prop"] /= n_cells
    intbc_counter = intbc_counter.sort_values(by="prop", ascending=False)
    intbc_counter.index = intbc_counter["iBC"]

    return lg_group, intbc_counter
...
def plot_heatmap(dm, xlink, ind1, outputdir, outSizeLen, outSizeHeight, maxD=1):
    ...
    plt.savefig(outputdir + "/alleleTable.filtered_lineageGroups.pdf")
...
def assign_lineage_groups(alleletable, ind1, df_pivot_I, outputdir):
    ...
    lineageGrp2cellBC = zip(ind1, df_pivot_I.index.tolist())
...
def cluster_with_dtcut(ds, minClusterSize=1):
    base = importr("base")
    stats = importr("stats")
    ...
    dmat = stats.as_dist(robjects.vectors.Matrix(ds))
    dendro = r.hclust(dmat, method="average")
    clusters = dtc.cutreeDynamic(dendro, distM = base.as_matrix(dmat), minClusterSize=minClusterSize,
...
def plot_overlap_heatmap(df_pivot_I, clusters, outputdir, cell_xlink):
    ...
    oMatIntBC = maxOverlap(df_pivot_I)
    dmIntBC = 1-oMatIntBC
...
def plot_overlap_heatmap_lg(lgs, at_pivot_I, outputdir):
    ...
    for i in tqdm(range(len(lgs))):
        ...
        lg_group_pivot = pd.pivot_table(lg_group, index=["cellBC"], columns=["intBC"], values=['s1', 's2', 's3'], aggfunc=pylab.unique).T
...
def collectAlleles(df, clusters, outputdir, thresh = 0.1):
    ...
    for i in tqdm(lineageGrps):
        lg_group, intbc_counter = get_lg_group(df, i)

        p_bc = intbc_counter[(intbc_counter["prop"] > thresh) & (intbc_counter["iBC"] != "NC")]

        lg_group["filter"] = "pass"
        for i in lg_group.index:
            if not lg_group.loc[i, "intBC"] in p_bc["iBC"]:
                lg_group.loc[i, "filter"] = "fail"

        lg_group = lg_group[(lg_group["filter"] == "pass")]
        lgs.append(lg_group)
...
def filteredLG2AT(filtered_lgs):
    ...
    final_df = final_df.groupby(["cellBC", "intBC", "allele", "r1", "r2", "r3", "lineageGrp"], as_index=False).agg({"UMI": "count"})
    final_df["Sample"] = "N"
...
if __name__ == "__main__":
    ...
    min_allele_thresh = params["min_allele_thresh"][0]
    ...
    with open(outputdir + "/lglog.txt", "a") as f:
        f.write(str(at_pivot.shape[0]) + " cells and " + str(at_pivot.shape[1]) + " intBCs\n")

    at_pivot_I = at_pivot
    at_pivot_I[at_pivot_I > 0] = 1

    print(">>> CALCULATING MAXIMUM OVERLAP MATRIX...")
    oMat = maxOverlap(at_pivot_I.T)
    ...
    clusters = cluster_with_dtcut(ds, minClusterSize=min_cluster_size)
